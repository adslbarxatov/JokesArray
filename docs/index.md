# Проект JokesArray: общие сведения
...
- [Мастер-канал сообщества](https://t.me/jokesarray)
...
за семь лет запас доступен на [нашем канале](https://t.me/jokesarray) вместе с новыми отечественными и иностранными текстами.

Казалось бы, зачем это нужно? А всё-таки результат того сто́ит. Контент от этих манипуляций становится только
...
Вещание сообщества уже давно ведётся в Telegram, используя менее удобную платформу
...
Кроме того, мы создали [приложение-клиент](https://adslbarxatov.github.io/DPArray/ru#jokesarray) для этого сообщества. Его главная особенность – способность извлекать
записи в случайном порядке из всего архива, исключая повторения, а также быстро и просто ими делиться.
Оно исключает необходимость ручного пролистывания записей в порядке их публикации. Кроме того, в нём теперь доступны
запрос записей по категориям и оффлайн-режим (работа без подключения к интернету со встроенной ограниченной копией
архива).

***Итак, добро пожаловать на JokesArray!***
...
чьего-нибудь копирайта, мы просим извещать нас об этом. Такой контент нам... будет изыматься из архива.
2. Записи могут быть разными. Конечно, без шока, интимных подробностей и прочей неразрешёнки. Но всё-таки `18+`!
3. Если мы всё же где-то допускаем ошибки (что вероятно), просим также нам об этом сообщать.
...
в комментариях [головного сообщества](https://t.me/rd_aaow_fdl) или в меню «Предложить запись» приложения-клиента.
...
Мы уже достаточно давно сопровождаем сообщество JokesArray. И на данный момент результатом
...
- Далее, в приложение [UniNotifier](https://adslbarxatov.github.io/UniNotifier) была добавлена функция *запроса к JokesArray*.
...
- Текстовые записи *не должны превышать тысячи символов*: таково техническое ограничение механизма в его текущей форме. В оффлайн-режиме неактуально.
...
- Реклама.
- Снова реклама (напомним, что JokesArray, согласно Интегрированной операционной политике, – полностью
...
Здесь можно выбрать необходимые параметры его работы.
...
<center><img src="/JokesArray/img/JA07.png" width="250" /></center>
...
**`Журнал`** – основной раздел приложения, который теперь будет первым появляться
при его запуске. Под ним расположена центральная кнопка «Ещё!». Первое нажатие отобразит вступительное слово, а каждое
последующее будет запрашивать одну случайную запись из сообщества (более 4050 уникальных постов на 1.02.2026).
...
- Нажав кнопку `Зап.` в нижнем правом углу журнала, можно быстро перейти в Telegram-канал JokesArray или поделиться текстом любым доступным
на устройстве способом.

<center><img src="/JokesArray/img/JA08.png" width="250" /></center>

Собственно, всё. Использование приложения требует лишь наличия подключения к интернету (или не требует, если включён оффлайн-режим)
и версии Android не ниже 5.0. Всё остальное – по Вашему усмотрению.
